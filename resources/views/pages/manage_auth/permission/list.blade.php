...
@section('scripts')
<script>
  let header = JSON.parse(`{!! json_encode($header) !!}`)
  let allow = JSON.parse(`{!! $allow !!}`)
  let _url = {
    datatable: `{{ route($module.'.datatable') }}`,
    create: `{{ route($module.'.create') }}?module_id=${header.id}`,
  ...
  $(() => {
    ...
    table = $(".dt-wow").DataTable({
      ...
      columns: [
        { searchable: false, orderable: false, name: 'id', data: 'id', className: 'text-center',
          render: (data, type, row) => {
            html = `<div class="btn-group btn-group-sm" branch="group">
              ...
              ${allow.permission ? `<a href="${_url.permission}" class="btn btn-sm btn-default" title="Permission"><i class="fas fa-stream"></i></a>`: '' }
            </div>`
        ...
        { searchable: true, orderable: true, name: 'name', data: 'name',
          render: (data, type, row) => {
            return data
          }
        },
        { searchable: true, orderable: true, name: 'description', data: 'description',
          render: (data, type, row) => {
            return data
          }
        },
        { searchable: true, orderable: true, name: 'created_at', data: 'created_at',
          render: (data, type, row) => {
            if(data == null){
              return "-";
            }
        ...
        { searchable: true, orderable: true, name: 'updated_at', data: 'updated_at',
          render: (data, type, row) => {
            if(data == null){
              return "-";
            }
