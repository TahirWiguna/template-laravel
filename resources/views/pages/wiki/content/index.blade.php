...
<div class="col-lg-6 col-7">
  ...
  <nav aria-label="breadcrumb" class="d-none d-md-inline-block ml-md-4">
    <ol class="breadcrumb breadcrumb-links breadcrumb-dark">
      ...
      <li class="breadcrumb-item"><a href="{{ route('wiki_content.index') }}">Header</a></li>
...
<script>
  const wiki_header_id = '{{ $wiki_header_id }}'

  function create(){
    window.location.href = "{{ route('wiki_content.create') }}" + "?wiki_header_id=" + wiki_header_id
  }
  ...
  function show(){
    ...
    window.location.href = "{{ route('wiki_content.show', ':id') }}".replace(':id', id)
  }
  ...
  function update(){
    ...
    window.location.href = "{{ route('wiki_content.edit', ':id') }}".replace(':id', id)
  }
  ...
  function destroy(){
    ...
    Swal.fire({
      ...
      if (result.value) {
        $.ajax({
          url: "{{ route('wiki_content.destroy', ':id') }}".replace(':id', id),
          headers: {
