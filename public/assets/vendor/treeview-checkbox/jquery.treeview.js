function TreeView(datas, options) {
    ...
    function changeCheckState(value, allChildCheck) {
        ...
        if (value == null || value instanceof MouseEvent) {
            // TOGGLE CHECK
            if (c == 0) c = 1;
            else if (c == 1) c = 0;
            else if (c == 2) c = 1;
        ...
        if (c != 2) checkAllChilds.bind(this)(c);
    ...
    function setCheckState(value) {
        ...
        if (value == 1) {
            $(this).find(">[check-icon]")[0].className = "fa fa-check-circle";
        }
        if (value == 2) {
            $(this).find(">[check-icon]")[0].className = "fa fa-dot-circle";
        }
    }
    ...
    function saveSingle(el) {
        if (el == null) el = this;
        ret = Object.assign({ children: [] }, el.data, { checked: el.checked });
